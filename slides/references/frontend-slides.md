# Slides Visual Reference Guide
> Extracted from ~/.claude/skills/frontend-slides/ — snapshot for slides-maker agent.
...
**Data / code / labels:** `JetBrains Mono` (JetBrains)
...
**DO NOT USE as display fonts:** Inter, Roboto, Arial, or system fonts — these produce generic "AI slop" aesthetics.

---

## clamp() Responsive Sizing Rules

All sizes use `clamp()` — never fixed `px` values for typography or spacing:

```css
/* Syntax: clamp(minimum, preferred-viewport-relative, maximum) */
--title-size: clamp(2rem, 5.5vw, 4rem);
--body-size:  clamp(0.75rem, 1.3vw, 1rem);
--small-size: clamp(0.6rem, 0.95vw, 0.8rem);
```

### CSS Gotcha — Negating clamp()
...
CSS does not allow a leading `-` before function names like `clamp()`, `min()`, `max()`. The browser silently discards the declaration — no console error.
...
Use Intersection Observer to add `.visible` class when slide enters viewport — do NOT use scroll events.
...
- Tab text must scale: `font-size: clamp(0.5rem, 1vh, 0.7rem)`
